...
<div class="client-locations-page">
    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <div class="stat-content">
                ...
                <p class="stat-number">{{ $stats['actives'] ?? 0 }}</p>
            </div>
        </div>
        <div class="stat-card">
            <div class="stat-icon">📋</div>
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                ...
                <p class="stat-number">{{ $stats['prochain_date'] ?? 'Aucun' }}</p>
                @if($stats['prochain_montant'] ?? false)
                    <span class="stat-sub">{{ number_format($stats['prochain_montant'], 0, ',', ' ') }} FCFA</span>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="locations-grid" id="locations-list">
        ...
        @endphp
        <div class="location-card" data-status="{{ $location->statut }}" data-id="{{ $location->id }}">
            <div class="location-image" onclick="openGallery({{ json_encode($bien->photos_array ?? []) }})">
                <img src="{{ asset($photo) }}" alt="{{ $bien->nom ?? 'Bien' }}">
                <span class="location-type {{ $type }}">
            ...
            <div class="location-details">
                <div class="location-header">
                    ...
                    <span class="status-badge status-{{ $location->statut }}">
                        @if($location->statut == 'acceptee') Active
                        @elseif($location->statut == 'en_attente') En attente
                        @else Terminée
                        @endif
                    </span>
                </div>
                ...
                <div class="location-address">
                    <span class="icon">📍</span> {{ $bien->adresse ?? '' }}, {{ $bien->ville ?? '' }}
                </div>

                <div class="info-grid">
                    <div class="info-item">
                        <span class="label">Début</span>
                        <span class="value">{{ $location->date_debut ? $location->date_debut->format('d/m/Y') : 'N/A' }}</span>
                    </div>
                    <div class="info-item">
                        <span class="label">Loyer</span>
                        <span class="value">{{ number_format($prix, 0, ',', ' ') }} FCFA/mois</span>
                    </div>
                    <div class="info-item">
                        <span class="label">Propriétaire</span>
                        <span class="value">{{ $proprietaire->nom ?? 'N/A' }}</span>
                    </div>
                    <div class="info-item">
                        <span class="label">Prochain paiement</span>
                        <span class="value highlight">{{ $location->prochain_paiement ?? ($location->date_debut ? $location->date_debut->addMonth()->format('d/m/Y') : 'N/A') }}</span>
                    </div>
                </div>

                <div class="location-actions">
                    @if($location->statut == 'acceptee')
                    <button class="btn-payer"
                            data-location-id="{{ $location->id }}"
                            data-type="{{ $type }}"
                            data-prix="{{ $prix }}"
                            data-bien-nom="{{ $bien->nom ?? ('Appartement ' . ($bien->numero_appartement ?? '')) }}"
                            data-prochain-paiement="{{ $location->prochain_paiement ?? ($location->date_debut ? $location->date_debut->addMonth()->format('d/m/Y') : '') }}"
                            onclick="openPaymentModal(this)">
                        <span class="btn-icon">💰</span>
                        <span class="btn-text">Payer le loyer</span>
                    </button>
                    @endif
                    <button class="btn-details" onclick="showLocationDetails({{ $location->id }})">
                        <span class="btn-icon">📋</span>
                        <span class="btn-text">Détails</span>
                    </button>
                    <button class="btn-contact" onclick="contacterProprietaire({{ $proprietaire->id ?? 0 }}, {{ $location->id }}, '{{ $type }}')">
                        <span class="btn-icon">✉️</span>
                        <span class="btn-text">Contacter</span>
                    </button>
                    <button class="btn-history" onclick="showPaiementHistory({{ $location->id }})">
                        <span class="btn-icon">📊</span>
