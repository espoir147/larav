    ...
    <div class="stats-grid">
        <div class="stat-card">
            ...
            <div class="stat-content">
                ...
                <p class="stat-number">{{ $stats['locations_actives'] }}</p>
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                <h3>Total mensuel</h3>
                <p class="stat-number">{{ number_format($stats['total_mensuel'], 0, ',', ' ') }} FCFA</p>
                <div class="stat-details">
        ...
        <div class="stat-card">
            ...
            <div class="stat-content">
                ...
                <p class="stat-number">{{ $stats['prochain_paiement'] ?? 'Aucun' }}</p>
                <div class="stat-details">
                    @if($stats['prochain_montant'] > 0)
                    <span class="detail-item">{{ number_format($stats['prochain_montant'], 0, ',', ' ') }} FCFA</span>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="locations-list">
            ...
            <div class="location-card">
                <div class="location-header">
                    <h3>{{ $bien->nom ?? 'Appartement ' . ($bien->numero_appartement ?? '') }}</h3>
                ...
                <div class="location-content">
                    <div class="location-info">
                        <div class="info-row">
                            ...
                            <span class="info-value">{{ $bien->adresse }}, {{ $bien->ville }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Propriétaire</span>
                            <span class="info-value">{{ $proprietaire->nom ?? 'N/A' }}</span>
                        </div>
                        @if($proprietaire->telephone ?? false)
                        <div class="info-row">
                            <span class="info-label">Téléphone</span>
                            <span class="info-value">📞 {{ $proprietaire->indicatif_pays ?? '' }} {{ $proprietaire->telephone ?? '' }}</span>
                        </div>
                        @endif
                        <div class="info-row">
                            <span class="info-label">Caractéristiques</span>
                            <span class="info-value">🛏️ {{ $bien->nombre_chambres }} chambre(s) • 🏠 {{ ucfirst($bien->type) }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Depuis le</span>
                            <span class="info-value">{{ $location->date_debut->format('d/m/Y') }}</span>
                        </div>
                    </div>
                    ...
                    <div class="location-actions">
                        <div class="price-box">
                            ...
                            <span class="price-value">{{ number_format($prix, 0, ',', ' ') }} FCFA</span>
                        </div>

                        <div class="action-buttons">
                            <button class="btn-secondary" onclick="showLocationDetails({{ $location->id }})">
                                <span class="btn-icon">📋</span>
                                <span>Détails</span>
                            </button>
                            <button class="btn-primary" onclick="openPaymentModal({{ $location->id }}, '{{ $location->type_logement }}', {{ $prix }}, '{{ addslashes($bien->nom ?? $bien->numero_appartement) }}')">
                                <span class="btn-icon">💰</span>
                                <span>Payer</span>
                            </button>
                        </div>
                ...
                <div class="last-payment">
                    <div class="payment-info">
                        ...
                        <span class="payment-detail">{{ $dernierPaiement->date_paiement->format('d/m/Y') }} - {{ number_format($dernierPaiement->montant, 0, ',', ' ') }} FCFA</span>
                    </div>
                    <a href="{{ route('proprietaire.loyer.recu', $dernierPaiement->id) }}" class="btn-receipt">
                        <span class="btn-icon">📄</span>
                        <span>Reçu</span>
                    </a>
                </div>
        ...
        <div class="payments-list">
            ...
            <div class="payment-item">
                <div class="payment-date">
                    <span class="date-day">{{ $paiement->created_at->format('d/m/Y') }}</span>
                    <span class="date-period">{{ $paiement->mois_paye }} {{ $paiement->annee_paye }}</span>
                </div>
                <div class="payment-amount">
                    <span class="amount">{{ number_format($paiement->montant, 0, ',', ' ') }} FCFA</span>
                    <span class="operator">{{ $paiement->operateur->nom ?? 'Mobile Money' }}</span>
                </div>
                <div class="payment-action">
                    <a href="{{ route('proprietaire.loyer.recu', $paiement->id) }}" class="btn-receipt small">
                        <span class="btn-icon">📄</span>
                        <span>Reçu</span>
                    </a>
                </div>
            ...
            <form id="fedapay-form" onsubmit="initiatePayment(event)">
                ...
                <div id="mobile-money-fields">
                    ...
                    <div class="form-group">
                        ...
                        <input type="tel" id="telephone" name="telephone" class="form-input" placeholder="Ex: 0701234567" value="{{ Auth::user()->telephone ?? '' }}" required>
                    </div>
                </div>
